...
class CoVSeq(dnaseq.DnaSeqRaw):
    ...
    def host_reads_removal(self):
        ...
        for readset in self.readsets:
            ...
            readset_bam_host_removed_sorted = os.path.join(host_removal_directory,
                                                           readset.name + ".host_removed.sorted.bam")
            readset_bam_host_removed_sorted_index = os.path.join(host_removal_directory,
                                                                 readset.name + ".host_removed.sorted.bam.bai")
            readset_bam_host_removed_name_sorted = os.path.join(host_removal_directory,
                                                                readset.name + ".host_removed.nsorted.bam")
            ...
            jobs.append(
                concat_jobs([
                    ...
                    pipe_jobs([
                        bwa.mem(
                            ...
                            read_group="'@RG" + \
                                       ...
                                       ("\\tCN:" + global_conf.get('host_reads_removal',
                                                                   'sequencing_center') if global_conf.get(
                                           'host_reads_removal', 'sequencing_center', required=False) else "") + \
                                       ("\\tPL:" + global_conf.get('host_reads_removal',
                                                                   'sequencing_technology') if global_conf.get(
                                           'host_reads_removal', 'sequencing_technology',
                    ...
                    sambamba.sort(
                        ...
                        tmp_dir=global_conf.get('host_reads_removal', 'tmp_dir', required=True),
                        other_options=global_conf.get('host_reads_removal', 'sambamba_name_sort_other_options',
                                                      required=False)
                    ),
                    sambamba.index(
                        readset_bam_host_removed_sorted,
                        readset_bam_host_removed_sorted_index,
                        other_options=global_conf.get('host_reads_removal', 'sambamba_index_other_options',
                                                      required=False)
                    ),
                    samtools.bam2fq(
    ...
    def kraken_analysis(self):
        ...
        for readset in self.readsets:
            ...
            if readset.run_type == "PAIRED_END":
                candidate_input_files = [
                    [host_removal_file_prefix + ".host_removed.pair1.fastq.gz",
                     host_removal_file_prefix + ".host_removed.pair2.fastq.gz"]
                ]
                if readset.fastq1 and readset.fastq2:
                    candidate_input_files.append([readset.fastq1, readset.fastq2])
                [fastq1, fastq2] = self.select_input_files(candidate_input_files)
                unclassified_output = [kraken_out_prefix + ".unclassified_sequences_1.fastq",
                                       kraken_out_prefix + ".unclassified_sequences_2.fastq"]
                classified_output = [kraken_out_prefix + ".classified_sequences_1.fastq",
                                     kraken_out_prefix + ".classified_sequences_2.fastq"]
        ...
        for readset in self.readsets:
            ...
            if readset.run_type == "PAIRED_END":
                candidate_input_files = [
                    ...
                    [host_removal_file_prefix + ".host_removed.pair1.fastq",
                     host_removal_file_prefix + ".host_removed.pair2.fastq"]
                ]
                if readset.fastq1 and readset.fastq2:
                    candidate_input_files.append([readset.fastq1, readset.fastq2])
                [fastq1, fastq2] = self.select_input_files(candidate_input_files)
 ...
 ($9 <= -{min_insert_size} && $9 >= -{max_insert_size})'""".format(
                                min_insert_size=global_conf.get('sambamba_filtering', 'min_insert_size',
                                                                required=False),
                        ...
                        sambamba.view(
                            ...
                            options=global_conf.get('sambamba_filtering', 'sambamba_filtering_other_options',
                                                    required=False)
                        )
                    ]),
                ],
    ...
    def ivar_trim_primers(self):
        ...
        for sample in self.samples:
            alignment_directory = os.path.join("alignment", sample.name)
            input_bam = os.path.join(alignment_directory, sample.name + ".sorted.filtered.bam")
            output_prefix = os.path.join(alignment_directory,
                                         re.sub("\.sorted.filtered.bam$", ".primerTrim", os.path.basename(input_bam)))
                    ...
                    concat_jobs([
                        ...
                        gatk4.collect_multiple_metrics(
                            ...
                            os.path.join(picard_directory,
                                         re.sub("\.bam$", "", os.path.basename(input)) + ".all.metrics"),
                ...
                jobs.append(
                    concat_jobs([
                        ...
                        gatk4.collect_oxog_metrics(
                            input,
                            os.path.join(picard_directory,
                                         re.sub("\.bam$", "", os.path.basename(input)) + ".oxog_metrics.txt")
                        )
                    ],
                ...
                jobs.append(
                    concat_jobs([
                        ...
                        gatk4.collect_gcbias_metrics(
                            input,
                            os.path.join(picard_directory,
                                         re.sub("\.bam$", "", os.path.basename(input)) + ".qcbias_metrics.txt"),
                            os.path.join(picard_directory,
                                         re.sub("\.bam$", "", os.path.basename(input)) + ".qcbias_metrics.pdf"),
                            os.path.join(picard_directory,
                                         re.sub("\.bam$", "", os.path.basename(input)) + ".qcbias_summary_metrics.txt")
                        )
                    ],
    ...
    def freebayes_calling(self):
        ...
        for sample in self.samples:
            ...
            for flag in ["ambiguous", "fixed"]:
                output_vcf_gz = re.sub(r"\.consensus\.norm\.vcf\.gz$", ".%s.norm.vcf.gz" % (flag),
    ...
    def snpeff_annotate(self):
        ...
        for sample in self.samples:
            variant_directory = os.path.join("variant", sample.name)
            metrics_ivar_prefix = os.path.join("metrics", "dna", sample.name, "snpeff_metrics_ivar",
                                               sample.name + ".snpEff")
            metrics_freebayes_prefix = os.path.join("metrics", "dna", sample.name, "snpeff_metrics_freebayes",
                                                    sample.name + ".snpEff")
            ...
            ivar_output_vcf = os.path.join(variant_directory,
                                           re.sub("\.vcf.gz$", ".annotate.vcf", os.path.basename(ivar_vcf)))

            freebayes_vcf = os.path.join(variant_directory, sample.name) + ".freebayes_calling.fixed.norm.vcf.gz"
            freebayes_output_vcf = os.path.join(variant_directory,
                                                re.sub("\.vcf.gz$", ".annotate.vcf", os.path.basename(freebayes_vcf)))
    ...
    def ivar_create_consensus(self):
        ...
        for sample in self.samples:
            ...
            output_prefix = os.path.join(consensus_directory,
                                         re.sub("\.bam$", ".consensus", os.path.basename(input_bam)))
...
sed s/{reference_genome_name}/{sample_name}/ > {output_consensus_fasta}""".format(
    ...
    def rename_consensus_header_ivar(self):
        ...
        for sample in self.samples:
            ...
            ivar_output_status_fa = os.path.join(consensus_directory,
            ...
            ivar_annotated_vcf = os.path.join(variant_directory,
                                              re.sub("\.vcf.gz$", ".annotate.vcf", os.path.basename(ivar_vcf)))
            ...
            bedgraph_file = os.path.join(alignment_directory,
                                         re.sub("\.bam$", ".BedGraph", os.path.basename(input_bam)))
...
ln -sf {ivar_output_status_fa_basename} {ivar_output_fa}""".format(
    ...
    def rename_consensus_header_freebayes(self):
        ...
        for sample in self.samples:
            ...
            freebayes_output_fa = os.path.join(consensus_directory,
                                               sample.name) + ".freebayes_calling.consensus.renamed.fasta"
            freebayes_output_status_fa = os.path.join(consensus_directory,
                                                      """{sample_name}.freebayes_calling.consensus.{technology}.{status}.fasta""".format(
                                                          sample_name=sample.name,
                                                          technology=global_conf.get('rename_consensus_header',
                                                                                     'sequencing_technology',
                                                                                     required=False),
                                                          status="${FREEBAYES_STATUS}"))

            variant_directory = os.path.join("variant", sample.name)
            freebayes_vcf = os.path.join(variant_directory, sample.name) + ".freebayes_calling.fixed.norm.vcf.gz"
            freebayes_annotated_vcf = os.path.join(variant_directory, re.sub("\.vcf.gz$", ".annotate.vcf",
                                                                             os.path.basename(freebayes_vcf)))
...
ln -sf {freebayes_output_status_fa_basename} {freebayes_output_fa}""".format(
    ...
    def ncovtools_quickalign(self):
        ...
        for sample in self.samples:
            ...
            freebayes_consensus = os.path.join(consensus_directory,
                                               sample.name) + ".freebayes_calling.consensus.renamed.fasta"
            jobs.append(
                concat_jobs([
    ...
    def prepare_table(self):
        ...
        for readset in self.readsets:
            ...
            if readset.run_type == "PAIRED_END":
                ...
                kraken_outputs.extend(
                    (unclassified_output_1, unclassified_output_2, classified_output_1, classified_output_2))
        ...
        for sample in self.samples:
            ...
            for input_bam in input_bams:
                flagstat_outputs.append(
                    os.path.join(flagstat_directory, re.sub("\.bam$", ".flagstat", os.path.basename(input_bam))))
            # Finding BedGraph file
            [input_bam] = self.select_input_files([
                [os.path.join(alignment_directory, sample.name + ".sorted.filtered.bam")],
                [os.path.join(alignment_directory, sample.name + ".sorted.bam")]
            ])
            bedgraph_outputs.append(
                os.path.join(alignment_directory, re.sub("\.bam$", ".BedGraph", os.path.basename(input_bam))))

            # Picard collect multiple metrics outputs
            picard_directory = os.path.join("metrics", "dna", sample.name, "picard_metrics")
            picard_out = os.path.join(picard_directory,
                                      re.sub("\.bam$", "", os.path.basename(input_bam)) + ".all.metrics")
...
echo -e "sample\\tct\\tdate" > {ivar_metadata}""".format(
        ...
        for sample in self.samples:
            ...
            ivar_output_primer_trimmed_bam = os.path.join(ivar_ncovtools_data_directory,
                                                          os.path.basename(primer_trimmed_bam))
            ...
            job = concat_jobs([
                ...
                Job(
                    ...
                    output_files=[ivar_output_filtered_bam, ivar_output_primer_trimmed_bam, ivar_output_consensus,
...
module load {R_covseqtools}""".format(
                        R_covseqtools=global_conf.get('prepare_report', 'module_R') + " " + global_conf.get(
                            'prepare_report', 'module_CoVSeQ_tools'),
                        output_dir=self.output_dir)
                    ),
...
echo -e "sample\\tct\\tdate" > {freebayes_metadata}""".format(
        ...
        for sample in self.samples:
            ...
            freebayes_output_filtered_bam = os.path.join(freebayes_ncovtools_data_directory,
                                                         os.path.basename(filtered_bam))
            freebayes_output_primer_trimmed_bam = os.path.join(freebayes_ncovtools_data_directory,
                                                               os.path.basename(primer_trimmed_bam))
            freebayes_output_consensus = os.path.join(freebayes_ncovtools_data_directory,
                                                      os.path.basename(freebayes_consensus))
            freebayes_output_variants = os.path.join(freebayes_ncovtools_data_directory,
                                                     os.path.basename(freebayes_variants))

            job = concat_jobs([
                job,
                Job(
                    input_files=[filtered_bam, primer_trimmed_bam, freebayes_consensus, freebayes_variants],
                    output_files=[freebayes_output_filtered_bam, freebayes_output_primer_trimmed_bam,
                                  freebayes_output_consensus, freebayes_output_variants],
                    command="""\\
echo "Linking files for ncov_tools for sample {sample_name}..." && \\
