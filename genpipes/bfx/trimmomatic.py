...
def trimmomatic(
    ...
    headcrop_length = global_conf.get('trimmomatic', 'headcrop_length', required=False, param_type='posint')
...
java {java_other_options} -Xmx{ram} -jar $TRIMMOMATIC_JAR {mode} \\
  ...
  2> {trim_log}""".format(
        java_other_options=global_conf.get('trimmomatic', 'java_other_options'),
        ram=global_conf.get('trimmomatic', 'ram'),
        mode = "PE" if input2 else "SE",
        threads=global_conf.get('trimmomatic', 'threads', param_type='posint'),
        quality_offset=quality_offset if quality_offset == 64 else "33",
        inputs=" \\\n  ".join(inputs),
        outputs=" \\\n  ".join(outputs),
        adapter_file=adapter_file,
        illumina_clip_settings=global_conf.get('trimmomatic', 'illumina_clip_settings'),
...
java -XX:ParallelGCThreads=1 -Xmx{ram} -jar $TRIMMOMATIC_JAR {mode} \\
  ...
  2> {trim_log}""".format(
        ram=global_conf.get('trimmomatic16S', 'ram'),
        mode = "PE" if input2 else "SE",
        threads=global_conf.get('trimmomatic16S', 'threads', param_type='posint'),
        quality_offset=quality_offset if quality_offset == 64 else "33",
        inputs=" \\\n  ".join(inputs),
        outputs=" \\\n  ".join(outputs),
