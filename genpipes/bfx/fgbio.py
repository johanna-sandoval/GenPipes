...
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $FGBIO_JAR AnnotateBamWithUmis \\
  ...
  {other_options}""".format(
            tmp_dir=global_conf.get('fgbio_addumi', 'tmp_dir'),
            java_other_options=global_conf.get('fgbio_addumi', 'java_other_options'),
            ram=global_conf.get('fgbio_addumi', 'ram'),
            input_bam=input_bam,
            input_umi=input_umi,
            output_bam=output_bam,
            other_options=global_conf.get('fgbio_addumi', 'other_options') if global_conf.get('fgbio_addumi', 'other_options', required=False) else ""
            ),
...
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $FGBIO_JAR TrimPrimers \\
  ...
  {hard_clip} \\
  {other_options}""".format(
            tmp_dir=global_conf.get('fgbio_trim_primers', 'tmp_dir'),
            java_other_options=global_conf.get('fgbio_trim_primers', 'java_other_options'),
            ram=global_conf.get('fgbio_trim_primers', 'ram'),
            input_bam=input_bam,
            primers=global_conf.get('fgbio_trim_primers', 'primers'),
            output_bam=output_bam,
            hard_clip="-H" if hard_clip else "",
            other_options=global_conf.get('fgbio_trim_primers', 'other_options')
            )
        )
