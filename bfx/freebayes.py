...
freebayes -f {reference_genome} \\
  ...
  | sed s/QR,Number=1,Type=Integer/QR,Number=1,Type=Float/ > {output_file}""".format(
    reference_genome=global_conf.get(ini_section, 'genome_fasta', param_type='filepath'),
    bed_targets="-t " + global_conf.get(ini_section, 'bed_targets', param_type='filepath', required=False) if global_conf.get(ini_section, 'bed_targets', required=False) else "",
    options=options if options else "",
    other_options=global_conf.get(ini_section, 'freebayes_options'),
    input_bam=input_bam,
...
process_gvcf.py {other_options} -m {output_masks} -v {output_ambiguous} -c {output_consensus} {intput_gvcf}""".format(
            other_options=global_conf.get(ini_section, 'process_gvcf_options'),
