...
filterm4.py {outfile} > {outfile_filtered} 2> {outfile_filtered}.log'""".format(
            ...
            outfile=outfile,
            m=global_conf.get('smrtanalysis_blasr', 'm', param_type='int'),
            threads=global_conf.get('smrtanalysis_blasr', 'threads', param_type='posint'),
            bestn=global_conf.get('smrtanalysis_blasr', 'bestn', param_type='int'),
            n_candidates=global_conf.get('smrtanalysis_blasr', 'n_candidates', param_type='int'),
            min_read_length=global_conf.get('smrtanalysis_blasr', 'min_read_length', param_type='int'),
            max_score=global_conf.get('smrtanalysis_blasr', 'max_score', param_type='int'),
            max_lcp_length=global_conf.get('smrtanalysis_blasr', 'max_lcp_length', param_type='int'),
            sam=" \\\n  -sam" if sam else "",
...
def filtering(fofn, input_xml, params_xml, output_dir, log):
    ...
    ref_params_xml = global_conf.get('smrtanalysis_filtering', 'filtering_settings')
    ...
    white_path = global_conf.get('smrtanalysis_filtering', 'whitelist_path', required=False)

    temp_dir = global_conf.get('smrtanalysis_filtering', 'tmp_dir')
    if (global_conf.get('smrtanalysis_filtering', 'tmp_dir').startswith("$")):
...
prinseq-lite.pl \\
  ...
  -out_good {output_dir}/data/filtered_subreads""".format(
            ...
            input_xml=input_xml,
            min_subread_length=global_conf.get('smrtanalysis_filtering', 'min_subread_length'),
            min_read_length=global_conf.get('smrtanalysis_filtering', 'min_read_length'),
            min_qual=global_conf.get('smrtanalysis_filtering', 'min_qual'),
            ...
            threads=global_conf.get('smrtanalysis_filtering', 'threads'),
            tmp_dir=temp_dir,
...
m4topre.py \\
  ...
  > {outfile}'""".format(
            ...
            allm4=allm4,
            subreads=subreads,
            bestn=global_conf.get('smrtanalysis_m4topre', 'bestn', param_type='int'),
            outfile=outfile
    ))
...
def pbalign(
    ...
    temp_dir = os.path.join(global_conf.get('smrtanalysis_filtering', 'tmp_dir'), sample_cutoff_mer_size_polishing_round)
    if (global_conf.get('smrtanalysis_pbutgcns', 'tmp_dir').startswith("$")):
        temp_dir = temp_dir.replace('$', '$SMRT_ORIGUSERENV_')
        temp_dir = temp_dir.replace('{', '')
        temp_dir = temp_dir.replace('}', '')
...
pbalign \\
  ...
  --regionTable={control_regions_fofn}'""".format(
            ...
            cmph5=cmph5,
            tmp_dir=temp_dir,
            threads=global_conf.get('smrtanalysis_pbalign', 'threads', param_type='posint'),
...
awk '{{if ($0~/>/) {{sub(/>/,"@",$0);print;}} else {{l=length($0);q=""; while (l--) {{q=q "9"}}printf("%s\\n+\\n%s\\n",$0,q)}}}}' {outfile} \\
  > {outfile_fastq}""".format(
            threads=global_conf.get('smrtanalysis_pbdagcon', 'threads', param_type='posint'),
            infile=infile,
            outfile=outfile,
...
def pbutgcns(assembly_directory, sample_cutoff_mer_size, mer_size_directory):
    ...
    temp_dir = os.path.join(global_conf.get('smrtanalysis_filtering', 'tmp_dir'), sample_cutoff_mer_size)
    if (global_conf.get('smrtanalysis_pbutgcns', 'tmp_dir').startswith("$")):
        temp_dir = temp_dir.replace('$', '$SMRT_ORIGUSERENV_')
        temp_dir = temp_dir.replace('{', '')
        temp_dir = temp_dir.replace('}', '')
...
pbutgcns_wf.sh'""".format(
            ...
            tmp_dir=temp_dir,
            prefix=prefix,
            threads=global_conf.get('smrtanalysis_pbutgcns', 'threads', param_type='posint'),
            outfile=outfile
    ))
...
gunzip -c \\
  ...
  > {outfile_fasta_uncompressed}""".format(
            protocol=global_conf.get('smrtanalysis_variant_caller', 'protocol', param_type='dirpath'),
            threads=global_conf.get('smrtanalysis_variant_caller', 'threads', param_type='posint'),
            algorithm=global_conf.get('smrtanalysis_variant_caller', 'algorithm'),
            cmph5=cmph5,
            ref_fasta=ref_fasta,
