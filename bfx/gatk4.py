...
def split_n_cigar_reads(
    ...
    if global_conf.get('gatk_split_N_trim', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --out {output}{intervals}{exclude_intervals}""".format(
            tmp_dir=global_conf.get('gatk_split_N_trim', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_split_N_trim', 'gatk4_java_options'),
            ram=global_conf.get('gatk_split_N_trim', 'ram'),
            other_options=global_conf.get('gatk_split_N_trim', 'other_options', required=False),
            reference_sequence=global_conf.get('gatk_split_N_trim', 'reference', param_type='filepath'),
            input=input,
            output=output,
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --output {output}""".format(
            tmp_dir=global_conf.get('gatk_mark_duplicates', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_mark_duplicates', 'gatk4_java_options'),
            ram=global_conf.get('gatk_mark_duplicates', 'ram'),
            remove_duplicates=remove_duplicates,
            inputs=" \\\n  ".join("--input " + input for input in inputs),
            threads=global_conf.get('gatk_mark_duplicates', 'threads', param_type='int'),
            output=output,
            metrics_file=metrics_file,
            max_records_in_ram=global_conf.get('gatk_mark_duplicates', 'max_records_in_ram', param_type='int')
        ),
        removable_files=[output, re.sub("\.([sb])am$", ".\\1ai", output), output + ".md5"]
    )

def base_recalibrator(input, output, intervals=None):
    if global_conf.get('gatk_base_recalibrator', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --spark-master local[{threads}] \\
  --output {output}""".format(
                tmp_dir=global_conf.get('gatk_base_recalibrator', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_base_recalibrator', 'gatk4_java_options'),
                options=global_conf.get('gatk_base_recalibrator', 'options'),
                ram=global_conf.get('gatk_base_recalibrator', 'ram'),
                threads=global_conf.get('gatk_base_recalibrator', 'threads', param_type='int'),
                input=input,
                intervals=" \\\n  --intervals " + intervals if intervals else "",
                reference_sequence=global_conf.get('gatk_base_recalibrator', 'genome_fasta', param_type='filepath'),
                known_dbsnp=global_conf.get('gatk_base_recalibrator', 'known_dbsnp', param_type='filepath'),
                known_gnomad=global_conf.get('gatk_base_recalibrator', 'known_gnomad', param_type='filepath'),
                known_mills=global_conf.get('gatk_base_recalibrator', 'known_mills', param_type='filepath'),
                output=output
            ),
...
def apply_bqsr(
    ...
    if global_conf.get('gatk_apply_bqsr', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --spark-master local[{threads}] \\
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_apply_bqsr', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_apply_bqsr', 'gatk4_java_options'),
            ram=global_conf.get('gatk_apply_bqsr', 'ram'),
            options=global_conf.get('gatk_apply_bqsr', 'options'),
            threads=global_conf.get('gatk_apply_bqsr', 'threads', param_type='int'),
            input=input,
...
def print_reads(
    ...
    if global_conf.get('print_reads', 'module_gatk').split("/")[2] < "4":
        return gatk.print_reads(
            input,
            output,
            base_quality_score_recalibration
...
def cat_variants(
    ...
    if global_conf.get('gatk_merge_vcfs', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --OUTPUT {output}""".format(
                tmp_dir=global_conf.get('gatk_merge_vcfs', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_merge_vcfs', 'gatk4_java_options'),
                ram=global_conf.get('gatk_merge_vcfs', 'ram'),
                options=global_conf.get('gatk_merge_vcfs', 'options'),
                reference=global_conf.get('gatk_merge_vcfs', 'genome_fasta', param_type='filepath'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  {stats} \\
  --output {output}""".format(
                tmp_dir=global_conf.get('gatk_merge_stats', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_merge_stats', 'gatk4_java_options'),
                ram=global_conf.get('gatk_merge_stats', 'ram'),
                options=global_conf.get('gatk_merge_stats', 'options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_cnn_score_variantss', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_cnn_score_variants', 'gatk4_java_options'),
            ram=global_conf.get('gatk_cnn_score_variants', 'ram'),
            options=global_conf.get('gatk_cnn_score_variants', 'options'),
            reference_sequence=global_conf.get('gatk_cnn_score_variants', 'reference', param_type='filepath'),
            input=input,
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --variant {input} \\
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_filter_variant_tranches', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_filter_variant_tranches', 'gatk4_java_options'),
            ram=global_conf.get('gatk_filter_variant_tranches', 'ram'),
            options=global_conf.get('gatk_filter_variant_tranches', 'options'),
            resource_hapmap=global_conf.get('gatk_cnn_score_variants', 'hapmap', param_type='filepath'),
            resource_mills=global_conf.get('gatk_cnn_score_variants', 'mills', param_type='filepath'),
            input=input,
            interval_list=" \\\n --intervals " + interval_list if interval_list else "",
            output=output
        )
...
#added interval_padding as a varibale. Because in chipseq we don't need to add any padding to the peaks
    ...
    if global_conf.get('gatk_haplotype_caller', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "{java_other_options} -Xmx{ram}" \\
  ...
  --output {output}{interval_padding} {interval_list}{intervals}{exclude_intervals}""".format(
                tmp_dir=global_conf.get('gatk_haplotype_caller', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_haplotype_caller', 'gatk4_java_options'),
                ram=global_conf.get('gatk_haplotype_caller', 'ram'),
                options=global_conf.get('gatk_haplotype_caller', 'options'),
                threads=global_conf.get('gatk_haplotype_caller', 'threads'),
                reference_sequence=global_conf.get('gatk_haplotype_caller', 'genome_fasta', param_type='filepath'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}{intervals}{exclude_intervals}""".format(
                tmp_dir=global_conf.get('gatk_combine_gvcf', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_combine_gvcf', 'gatk4_java_options'),
                ram=global_conf.get('gatk_combine_gvcf', 'ram'),
                other_options=global_conf.get('gatk_combine_gvcf', 'other_options', required=False),
                reference_sequence=global_conf.get('gatk_combine_gvcf', 'genome_fasta', param_type='filepath'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}{intervals}""".format(
                tmp_dir=global_conf.get('gatk_GenomicsDBImport', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_GenomicsDBImport', 'gatk4_java_options'),
                ram=global_conf.get('gatk_GenomicsDBImport', 'ram'),
                other_options=global_conf.get('gatk_GenomicsDBImport', 'other_options', required=False),
                input="".join(" \\\n  --variant " + input for input in inputs),
                output=output,
                intervals="".join(" \\\n  --intervals " + interval for interval in intervals),
        ))
...
def genotype_gvcf(
    ...
    if global_conf.get('gatk_genotype_gvcf', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_genotype_gvcf', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_genotype_gvcf', 'gatk4_java_options'),
            ram=global_conf.get('gatk_genotype_gvcf', 'ram'),
            options=options,
            reference_sequence=global_conf.get('gatk_genotype_gvcf', 'genome_fasta', param_type='filepath'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {outputVCF}{interval_list}{intervals}{exclude_intervals}""".format(
        tmp_dir=global_conf.get('gatk_mutect', 'tmp_dir'),
        java_other_options=global_conf.get('gatk_mutect2', 'gatk4_java_options'),
        ram=global_conf.get('gatk_mutect2', 'ram'),
        options=global_conf.get('gatk_mutect2', 'options'),
        reference_sequence=global_conf.get('gatk_mutect2', 'genome_fasta', param_type='filepath'),
        read_orientation=read_orientation,
        known_sites=global_conf.get('gatk_mutect2', 'known_sites', param_type='filepath'),
        ...
        interval_list=" \\\n  --interval-padding 100 --intervals " + interval_list if interval_list else "",
        intervals="".join(" \\\n  --intervals " + interval for interval in intervals),
        #pon=" --panel-of-normals " + global_conf.get('gatk_mutect2', 'pon', type='filepath') if global_conf.get('gatk_mutect2', 'pon', param_type='filepath') else "",
        exclude_intervals="".join(
            " \\\n  --exclude-intervals " + exclude_interval for exclude_interval in exclude_intervals)
        )
    )
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_get_pileup_summaries', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_get_pileup_summaries', 'gatk4_java_options'),
            ram=global_conf.get('gatk_get_pileup_summaries', 'ram'),
            options=global_conf.get('gatk_get_pileup_summaries', 'options'),
            variants=global_conf.get('gatk_get_pileup_summaries', 'known_sites', param_type='filepath'),
            intervals=global_conf.get('gatk_get_pileup_summaries', 'known_intervals', param_type='filepath'),
            input_bam=input_bam,
            output=output
        )
    )
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  {normal}{tumor_segment}""".format(
            tmp_dir=global_conf.get('gatk_calculate_contamination', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_calculate_contamination', 'gatk4_java_options'),
            ram=global_conf.get('gatk_calculate_contamination', 'ram'),
            options=global_conf.get('gatk_calculate_contamination', 'options'),
            input=input,
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  {input} \\
  --output {output}""".format(
                tmp_dir=global_conf.get('gatk_learn_read_orientation_model', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_learn_read_orientation_model', 'gatk4_java_options'),
                ram=global_conf.get('gatk_learn_read_orientation_model', 'ram'),
                options=global_conf.get('gatk_learn_read_orientation_model', 'options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
                tmp_dir=global_conf.get('gatk_filter_mutect_calls', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_filter_mutect_calls', 'gatk4_java_options'),
                ram=global_conf.get('gatk_filter_mutect_calls', 'ram'),
                options=global_conf.get('gatk_filter_mutect_calls', 'options'),
                reference=global_conf.get('gatk_filter_mutect_calls', 'genome_fasta', param_type='filepath'),
                variants=variants,
...
def variant_recalibrator(variants,
    ...
    if global_conf.get('gatk_variant_recalibrator', 'module_gatk').split("/")[2] < "4":
        ...
        if small_sample_check:
            try:
                small_sample_option = global_conf.get('gatk_variant_recalibrator', 'small_sample_option')
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --tranches-file {tranches_output}""".format(
            tmp_dir=global_conf.get('gatk_variant_recalibrator', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_variant_recalibrator', 'gatk4_java_options'),
            ram=global_conf.get('gatk_variant_recalibrator', 'ram'),
            options=global_conf.get('gatk_variant_recalibrator', 'options'),
            reference_sequence=global_conf.get('gatk_variant_recalibrator', 'genome_fasta', param_type='filepath'),
            variants="".join(" \\\n  --variant " + variant for variant in variants),
...
def apply_recalibration(
    ...
    if global_conf.get('gatk_apply_recalibration', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
                tmp_dir=global_conf.get('gatk_apply_recalibration', 'tmp_dir'),
                java_other_options=global_conf.get('gatk_apply_recalibration', 'gatk4_java_options'),
                ram=global_conf.get('gatk_apply_recalibration', 'ram'),
                options=global_conf.get('gatk_apply_recalibration', 'options'),
                reference_sequence=global_conf.get('gatk_apply_recalibration', 'genome_fasta', param_type='filepath'),
                variants=variants,
                other_options=other_options,
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --padding {padding} \\
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_processIntervals', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_processIntervals', 'gatk4_java_options'),
            ram=global_conf.get('gatk_processIntervals', 'ram'),
            options=global_conf.get('gatk_processIntervals', 'options'),
            reference_sequence=global_conf.get('gatk_processIntervals', 'genome_fasta', param_type='filepath'),
            intervals=" \\\n --intervals " + intervals if intervals else "",
            bin_length=global_conf.get('gatk_processIntervals', 'bin-length'),
            padding=global_conf.get('gatk_processIntervals', 'padding'),
            output=output
        )
    )
...
def build_bam_index(
    ...
    if global_conf.get('build_bam_index', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --OUTPUT {output} """.format(
                ...
                java_other_options=global_conf.get('build_bam_index', 'gatk4_java_options'),
...
def calculate_hs_metrics(
    ...
    if global_conf.get('picard_calculate_hs_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --REFERENCE_SEQUENCE {reference_sequence}""".format(
                ...
                java_other_options=global_conf.get('picard_calculate_hs_metrics', 'gatk4_java_options'),
...
def collect_multiple_metrics(
    ...
    if global_conf.get('picard_collect_multiple_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_collect_multiple_metrics', 'gatk4_java_options'),
...
def collect_sequencing_artifacts_metrics(
    ...
    if global_conf.get('picard_collect_sequencing_artifacts_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 CollectSequencingArtifactMetrics \\
  ...
  --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
            ...
            java_other_options=global_conf.get('picard_collect_sequencing_artifacts_metrics', 'gatk4_java_options'),
...
def convert_sequencing_artifacts_metrics(
    ...
    if global_conf.get('picard_convert_sequencing_artifacts_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --REFERENCE_SEQUENCE {reference}""".format(
                ...
                java_other_options=global_conf.get('picard_convert_sequencing_artifacts_metrics', 'gatk4_java_options'),
...
def collect_oxog_metrics(
    ...
    if global_conf.get('picard_collect_oxog_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_collect_oxog_metrics', 'gatk4_java_options'),
                ram=global_conf.get('picard_collect_oxog_metrics', 'ram'),
                input=input,
                output=output,
                dbsnp="--DB_SNP " + global_conf.get('picard_collect_oxog_metrics', 'known_variants') if global_conf.get('picard_collect_oxog_metrics', 'known_variants') else "",
...
def collect_gcbias_metrics(
    ...
    if global_conf.get('picard_collect_gcbias_metrics', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
            ...
            java_other_options=global_conf.get('picard_collect_gcbias_metrics', 'gatk4_java_options'),
            ram=global_conf.get('picard_collect_gcbias_metrics', 'ram'),
            input=input,
            output=output,
            chart=chart,
            summary_file=summary_file,
            reference=reference_sequence if reference_sequence else global_conf.get('picard_collect_gcbias_metrics',
                                                                                 'genome_fasta'),
...
def fix_mate_information(
    ...
    if global_conf.get('picard_fix_mate_information', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_fix_mate_information', 'gatk4_java_options'),
...
def picard_mark_duplicates(
    ...
    if global_conf.get('picard_mark_duplicates', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_mark_duplicates', 'gatk4_java_options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_mark_duplicates_mate_cigar', 'gatk4_java_options'),
...
def merge_sam_files(inputs,
    ...
    if global_conf.get('picard_merge_sam_files', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_merge_sam_files', 'gatk4_java_options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_reorder_sam', 'gatk4_java_options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --FASTQ {fastq}{second_end_fastq}""".format(
                ...
                java_other_options=global_conf.get('picard_sam_to_fastq', 'gatk4_java_options'),
...
def sort_sam(
    ...
    if global_conf.get(ini_section, 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get(ini_section, 'gatk4_java_options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
 ...
 --MAX_RECORDS_IN_RAM {max_records_in_ram}""".format(
                ...
                java_other_options=global_conf.get('picard_collect_rna_metrics', 'gatk4_java_options'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --MATRIX_OUTPUT {matrix}""".format(
                tmp_dir=global_conf.get('gatk_crosscheck_fingerprint', 'tmp_dir'),
                options=global_conf.get('gatk_crosscheck_fingerprint', 'options'),
                java_other_options=global_conf.get('gatk_crosscheck_fingerprint', 'gatk4_java_options'),
                haplotype_database=global_conf.get('gatk_crosscheck_fingerprint', 'haplotype_database'),
                lod_threshold=global_conf.get('gatk_crosscheck_fingerprint', 'lod_threshold'),
                ram=global_conf.get('gatk_crosscheck_fingerprint', 'ram'),
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --LOD_THRESHOLD {lod_threshold} \\
  --OUTPUT {output} """.format(
                tmp_dir=global_conf.get('gatk_cluster_crosscheck_metrics', 'tmp_dir'),
                options=global_conf.get('gatk_cluster_crosscheck_metrics', 'options'),
                java_other_options=global_conf.get('gatk_cluster_crosscheck_metrics', 'gatk4_java_options'),
                lod_threshold=global_conf.get('gatk_cluster_crosscheck_metrics', 'lod_threshold'),
                ram=global_conf.get('gatk_cluster_crosscheck_metrics', 'ram'),
                input=input,
                output=output,
            )
        )
...
def bed2interval_list(
    ...
    if global_conf.get('gatk_bed2interval_list', 'module_gatk').split("/")[2] < "4":
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --OUTPUT {output}""".format(
            tmp_dir=global_conf.get('gatk_bed2interval_list', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_bed2interval_list', 'gatk4_java_options'),
            ram=global_conf.get('gatk_bed2interval_list', 'ram'),
            dictionary=dictionary if dictionary else global_conf.get('gatk_bed2interval_list', 'genome_dictionary', param_type='filepath'),
            bed=bed,
            output=output
            )
        )
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --INPUT {input} \\
  --OUTPUT {output}""".format(
            tmp_dir=global_conf.get('gatk_interval_list2bed', 'tmp_dir'),
            java_other_options=global_conf.get('gatk_interval_list2bed', 'gatk4_java_options'),
            ram=global_conf.get('gatk_interval_list2bed', 'ram'),
            input=input,
            output=output
            )
        )
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --intervals {intervals} \\
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_preProcessInterval', 'tmp_dir'),
            options=options if options else global_conf.get('gatk_preProcessInterval', 'options'),
            java_other_options=global_conf.get('gatk_preProcessInterval', 'gatk4_java_options'),
            ram=global_conf.get('gatk_preProcessInterval', 'ram'),
            reference=reference if reference else global_conf.get('gatk_preProcessInterval', 'genome_fasta', param_type='filepath'),
            intervals=intervals,
#            exclude_intervals="".join(" \\\n  --excludeIntervals " + exclude_interval for exclude_interval in exclude_intervals),
            output=output
        )
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --intervals {intervals} \\
  --output {output}""".format(
            tmp_dir=global_conf.get('gatk_splitInterval', 'tmp_dir'),
            options=options if options else global_conf.get('gatk_splitInterval', 'options'),
            jobs=jobs,
            java_other_options=global_conf.get('gatk_splitInterval', 'gatk4_java_options'),
            ram=global_conf.get('gatk_splitInterval', 'ram'),
            reference=global_conf.get('gatk_splitInterval', 'genome_fasta', param_type='filepath'),
            intervals=intervals,
#            exclude_intervals="".join(" \\\n  --excludeIntervals " + exclude_interval for exclude_interval in exclude_intervals),
            output=output
        )
